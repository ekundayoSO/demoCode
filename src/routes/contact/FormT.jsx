...
const Contact = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const contentResponse = await fetchContent('node/contactpage?include=field_image');
        setContent(contentResponse.data[0]);
        setIndluded(contentResponse.included);

        const formResponse = await fetch(`${drupalLocalhostAddress}/jsonapi/block_content/mautic_block`);
        ...
        const mauticFormId = formData.data[0]?.attributes?.field_mautic_block_formid;
  ...
  useEffect(() => {
    if (content?.attributes?.body?.value) {
      setSanitizedDrupalContent(DOMPurify.sanitize(content.attributes.body.value));
    }
  }, [content]);
  ...
  const imageUrl = imageFile ? `${drupalLocalhostAddress}${imageFile.attributes.uri.url}` : null;

  return (
    <Section>
      {imageUrl && <HeroImage src={imageUrl} altText={content.relationships?.field_image?.data?.meta?.alt} />}

      {content && content.attributes && <SectionHeading>{content.attributes.title}</SectionHeading>}
